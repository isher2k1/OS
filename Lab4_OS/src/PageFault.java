...
public class PageFault {
  ...
  {
    int oldestPage = getMostNotRecentlyUsedPage(mem, virtPageNum);
    Page page = ( Page ) mem.elementAt( oldestPage );
  ...
  private static int getMostNotRecentlyUsedPage(Vector mem, int virtPageNum) {
    int biggestTime = -1;
    int pageNum = -1;
    for (int i = 0; i < virtPageNum; i++) {
      Page p = (Page) mem.elementAt(i);
      if (p.lastTouchTime > biggestTime )  {
        biggestTime = p.lastTouchTime;
        pageNum = i;
      }
    }
    return pageNum;
